...
import { WIZARD_STEPS } from '../types/vendor';
import { generateVendorId, prepareSubmissionData, toJsonFormat, downloadJson } from '../utils/vendorUtils';
...
export default function VendorWizard({ businessSupport }: VendorWizardProps) {
    ...
    switch (currentStep) {
      ...
      case 'product':
        if (!validators.required(formData.espProductSelected)) {
          missingFields.push(t('product.productFamily'));
        }
        ...
        } else if (formData.espProductSelected) {
          if (!formData.productVariant && !formData.productSoCVariant) {
            missingFields.push(t('product.moduleVariant') + ' / ' + t('product.socVariant'));
          }
        }
        if (missingFields.length > 0) {
          showMandatoryFieldsAlert(missingFields);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // TODO: Replace with actual API call to Langchao backend
      // await fetch('https://langchao-api.example.com/vendors', {
      //   method: 'POST',
      ...
      Message.success(t('validation.submitSuccess'));
      setCurrentStep('success');
    } catch (error) {
      console.error('Submission error:', error);
      Message.error(t('validation.submitError'));
    } finally {
